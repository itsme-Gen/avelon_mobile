...
export default function IDVerification() {
  ...
  const [selectedIdFront, setSelectedIdFront] = useState<string | null>(
    savedDocs.frontUri,
  );
  const [selectedIdBack, setSelectedIdBack] = useState<string | null>(
    savedDocs.backUri,
  );
  const [selectedSignature, setSelectedSignature] = useState<string | null>(
    savedDocs.signatureUri,
  );
  const [selectedIncome, setSelectedIncome] = useState<string | null>(
    savedDocs.proofOfIncomeUri,
  );
  ...
  const takePhoto = async (
    ...
    try {
      ...
      showAlert({
        title: "Camera Error",
        message:
          "Failed to open camera. Please try again or use the gallery option.",
        icon: "camera-outline",
        iconColor: "#EF4444",
        buttons: [{ text: "OK" }],
      });
  ...
  const pickImage = async (
    ...
    try {
      ...
      showAlert({
        title: "Gallery Error",
        message:
          "Failed to open gallery. Please try again or use the camera option.",
        icon: "images-outline",
        iconColor: "#EF4444",
        buttons: [{ text: "OK" }],
      });
  ...
  const handleNext = () => {
    if (!selectedIdType) {
      showAlert({
        ...
        message:
          "Please select the type of government-issued ID you are submitting.",
    ...
    if (!selectedIdFront || !selectedIdBack || !selectedSignature) {
      showAlert({
        ...
        message:
          "Please upload your ID (front & back) and e-signature before proceeding.",
        ...
        <View className="px-6 pt-8">
          ...
          <View className="mb-6">
            ...
            >
              ...
              >
                {selectedIdType
                  ? (ID_TYPE_OPTIONS.find((o) => o.value === selectedIdType)
          ...
          <View className="gap-4 mb-6">
            {renderUploadButton(
              "income",
              "Proof of Income (payslip, bank statement, ITR)",
              selectedIncome,
            )}
            {renderUploadButton(
              "address",
      ...
      >
        ...
        <View className="px-6 pb-4 flex-row gap-3">
          <TouchableOpacity
            ...
            onPress={() => router.back()}
          >
            <Text className="text-gray-900 font-semibold text-base">Back</Text>
          </TouchableOpacity>
